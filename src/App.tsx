  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main font-sans selection:bg-accent">
      ...
      <main className="lg:ml-[260px] p-4 lg:p-10 flex flex-col gap-6 lg:gap-8 technical-grid min-h-screen pb-24 lg:pb-10">
        {/* Header */}
        <header className="flex flex-col items-stretch glass-panel p-4 lg:p-6 rounded-2xl lg:rounded-3xl border border-white/5 shadow-2xl gap-4">
          ...
          {/* Mobile Service Quick Switcher */}
          <div className="lg:hidden flex flex-col gap-2 border-t border-white/5 pt-3 mt-1">
            <div className="flex items-center justify-between px-1">
              <span className="text-[9px] font-black text-accent uppercase tracking-[0.2em]">Seleção de Escala</span>
              <span className="text-[8px] text-text-muted animate-pulse">Deslize para ver todas →</span>
            </div>
            <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar no-scrollbar scroll-smooth pb-1 px-1">
              {services.map(s => (
                <button
                  key={s.id}
                  onClick={() => switchService(s.id)}
                  className={cn(
                    ...
                    activeServiceId === s.id 
                      ? "bg-accent text-bg-main border-accent shadow-[0_0_15px_rgba(197,160,89,0.3)]" 
              ...
              <button
                onClick={createNewService}
                className="flex-shrink-0 p-2 bg-accent/10 border border-accent/20 text-accent rounded-xl hover:bg-accent/20 transition-all"
              >
                <Plus className="w-4 h-4" />
              </button>
            </div>
